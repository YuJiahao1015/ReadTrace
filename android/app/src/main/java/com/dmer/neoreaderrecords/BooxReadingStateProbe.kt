...
object BooxReadingStateProbe {
    ...
    fun run(context: Context, reason: String = "manual"): Result {
        ...
        out.append('\n')
        probeStats(context, out, latest)
        out.append('\n')
        probeCurrentPageTextCandidates(context, out)
        out.append('\n')
        out.append("说明：此探测读取文石数据库最后写回状态，并尝试从统计/批注字段寻找当前页文本候选。若 Metadata 变化频繁且 OnyxStatisticsModel 最新 currPage/lastPage 同步变化，说明可近实时拿到页码；若 orgText/note/comment/quote 等候选为空，则当前接口暂未暴露整页文本。\n")
    ...
    private fun probeStats(context: Context, out: StringBuilder, latest: LatestBook?) {
